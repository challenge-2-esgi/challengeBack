...
const candidateService = (configService: ConfigService): ClientProvider => ({
  transport: Transport.TCP,
  options: {
    host: configService.get('CANDIDATE_SERVICE_HOST'),
    port: configService.get('CANDIDATE_SERVICE_PORT'),
  },
});

export { Services, authService, candidateService };
