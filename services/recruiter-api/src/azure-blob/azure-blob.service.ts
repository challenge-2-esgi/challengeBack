...
export class AzureBlobService {
  ...
  async uploadFiles(files: Express.Multer.File[], access: AccessType) {
    let blobUrls = [];
    try {
      const promises: Array<Promise<string>> = [];

      files.forEach((file) => {
        promises.push(this.uploadFile(file, access));
      });
      blobUrls = await Promise.all(promises);
    } catch (error) {
      throw new Error(error);
    }
  ...
  async deleteFiles(fileUrls: string[], access: AccessType) {
    try {
      const promises: Array<Promise<void>> = [];
      fileUrls.forEach((fileUrl) => {
        promises.push(this.deleteFile(fileUrl, access));
      });
